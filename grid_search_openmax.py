...
def experiment_setup(model_path, inlier_name, outlier_name, supervisor_name):
    ...
    if "openmax" in supervisor_name.lower(): from svc.ood.openmax import OpenMax as supervisor
    ...
    inlier_training = load_cifar10(batch_size=1, shuffle=False, transform=transform, train=True)

    for tail_length in [5, 10, 20, 50, 100]:
        for alpha in [1, 3, 5, 10]:
            t0 = time.time()
            sv = supervisor(model=net, device=device, training_set=inlier_training, alpha_rank=alpha, tail_length=tail_length)
            print("running with alpha: {}, and tail: {}".format(alpha, tail_length))
            ...
            with open(experiment_name, 'a+') as f:
                f.write("{}, {}, {}, {}\n".format(alpha, tail_length, auroc, fpr95))
            print("{}, {}, {}, {}. Completed in {}\n".format(alpha, tail_length, auroc, fpr95, time.time()-t0))
